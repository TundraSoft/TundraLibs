...
import type { CronusEvents, CronusJob, ScheduleFields } from './types/mod.ts';
import {
  CronusDuplicateJob,
...
export class Cronus extends Events<CronusEvents> {
   ...
   * @return Returns true if the job exists, false otherwise
   */
   ...
   * @returns Returns the CronusJob object if found
   * @throws CronusJobNotFound
   * Will throw CronusJobNotFound if a job is not found by the provided name.
   * Since cronus is not case sensitive, this would occur only if trying
   * to access a job before it is added.
   */
  public getJob(name: string): CronusJob {
    if (this.hasJob(name)) {
      return this._jobs.get(this._cleanJobName(name)) as CronusJob;
    }
    throw new CronusJobNotFound(name);
  }
   ...
   * @param jobDetails - An object containing details about the job
   * @returns Returns the current Cronus instance
   * @throws CronusJobAlreadyExists
   * If a job by the same name already exists, CronusJobAlreadyExists will be
   * thrown. Note that Cronus is not case sensitive, so a job with the name
   * "MyJob" will be considered the same as "myjob" or "MYJOB"
   * @throws CronusJobInvalidSchedule
   * If the schedule is not a valid cron syntax, CronusJobInvalidSchedule will
   * be thrown. See https://crontab.guru/ for help with cron syntax
   * @throws CronusJobInvalidCallback
   * If the callback is not a function, CronusJobInvalidCallback will be thrown
   */
  public addJob(name: string, jobDetails: CronusJob): Cronus {
    name = this._cleanJobName(name);
    if (this.hasJob(name)) {
      throw new CronusDuplicateJob(name);
    }
    jobDetails.schedule = Cronus._cleanSchedule(jobDetails.schedule);
    // Validate Schedule
    if (Cronus.validateSchedule(jobDetails.schedule) === false) {
      throw new CronusInvalidSchedule(name, jobDetails.schedule);
    }
    if (!(jobDetails.action instanceof Function)) {
      throw new CronusInvalidCallback(name, jobDetails.schedule);
    }
    // Add to jobs map
   ...
   * @returns Returns the Cronus instance
   */
   ...
   * @param jobDetails - The updated details for the job.
   * @returns The Cronus instance after updating the job.
   */
   ...
   * @returns The Cronus instance after enabling the job.
   */
   ...
   * @returns The Cronus instance after disabling the job.
   */
   ...
   *
   * @param schedule - The cron schedule to get jobs for.
   * @returns An array of job names for the given schedule. Empty array if schedule does not exist
   */
   ...
   * @returns Whether the expression can run at this moment.
   */
   ...
   * @returns Whether the schedule has a valid syntax.
   */
   ...
   *
   * @param name - The job name to clean.
   * @returns The cleaned-up job name.
   */
   ...
   * @returns The cleaned-up schedule.
   */
   ...
   * @param type - The type of cron field being checked.
   * @param currVal - The current value of the field (optional).
   * @returns Returns true if the field value is valid, false otherwise.
   */
   ...
   * @param from - The minimum allowed value for the range.
   * @param to - The maximum allowed value for the range.
   * @returns `true` if the value is a valid range, `false` otherwise.
   */
        ...
        Promise.allSettled(jobNames.map(async (name) => {
          ...
          try {
            ...
            if (error !== undefined) {
              ...
              // Call success event
              this.emit('success', id, name, start, timeTaken, output);
            }
            // Call run event
