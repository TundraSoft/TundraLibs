...
export abstract class AbstractClient<O extends ClientOptions = ClientOptions>
  ...
  constructor(name: string, options: OptionKeys<O, ClientEvents>) {
    if (options.dialect === undefined) {
      throw new DAMConfigError('Dialect is required', {
        config: name,
        configItem: 'dialect',
      });
    }
  ...
  public async connect() {
    ...
    try {
      ...
      if (e instanceof DAMBaseError) {
        ...
        nErr = new DAMClientError(e.message, {
          config: this.name,
  ...
  >(sql: string, params?: Record<string, unknown>): Promise<QueryResults<R>> {
    ...
    try {
      ...
      if (err instanceof DAMBaseError) {
        ...
        nErr = new DAMQueryError(err.message, { sql, params }, {
          config: this.name,
          dialect: this.dialect,
        });
      }
  ...
  public async execute(
    ...
    try {
      ...
      throw new DAMQueryError(err.message, { sql, params }, {
        config: this.name,
        dialect: this.dialect,
      });
    }
  ...
  protected _sanitizeQuery(
    ...
    if (missing.length > 0) {
      throw new DAMQueryError(`Missing parameters: ${missing.join(', ')}`, {
        sql,
        params,
      }, {
        config: this._name,
