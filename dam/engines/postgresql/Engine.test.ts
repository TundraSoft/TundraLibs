...
const TEST_CONFIG = {
  ...
  connectionTimeout: 2, // 2 seconds for faster tests
  queryTimeout: 2, // 2 seconds for faster tests
};

Deno.test('dam.engines.postgresql', async (t) => {
  ...
  await t.step('query execution', async (u) => {
    ...
    await u.step('should handle complex PostgreSQL data types', async () => {
      ...
      // Cleanup
      await pgEngine.execute({
        sql: 'DROP TABLE IF EXISTS test_complex_types',
      });
      await teardownPostgreSQL();
    });
    ...
    await u.step('should handle NULL values and edge cases', async () => {
      ...
      const edgeCases = [
        ...
        {
          sql: 'SELECT :param_null:::text as param_null',
          params: { param_null: null },
          expected: null,
        },
        {
          sql: 'SELECT :empty_string:::text as empty_string',
          params: { empty_string: '' },
          expected: '',
        },
        {
          sql: 'SELECT :zero_number:::integer as zero_number',
          params: { zero_number: 0 },
          expected: 0,
        },
        {
          sql: 'SELECT :false_boolean:::boolean as false_boolean',
          params: { false_boolean: false },
          expected: false,
        },
      ];
  ...
  await t.step('transaction management and concurrent scenarios', async (u) => {
    ...
    await u.step('should handle multiple concurrent transactions', async () => {
      ...
      // Drop and recreate test table to ensure clean state
      await pgEngine.execute({
        sql: 'DROP TABLE IF EXISTS test_concurrent_tx',
      });
      await pgEngine.execute({
        sql:
          'CREATE TABLE test_concurrent_tx (id SERIAL PRIMARY KEY, tx_name TEXT, value INTEGER)',
      });
      ...
      const operations = [
        pgEngine.execute({
          sql:
            'INSERT INTO test_concurrent_tx (tx_name, value) VALUES (:tx_name:, :value:)',
    ...
    await u.step('should handle transaction isolation levels', async () => {
      ...
      await pgEngine.execute({
        sql:
          'CREATE TABLE test_isolation (id SERIAL PRIMARY KEY, counter INTEGER DEFAULT 0)',
      });
    ...
    await u.step('should handle transaction deadlock detection', async () => {
      ...
      await pgEngine.execute({
        sql:
          'CREATE TABLE IF NOT EXISTS test_deadlock_a (id INTEGER PRIMARY KEY, value TEXT)',
      });
      await pgEngine.execute({
        sql:
          'CREATE TABLE IF NOT EXISTS test_deadlock_b (id INTEGER PRIMARY KEY, value TEXT)',
      });

      // Insert initial data
      await pgEngine.execute({
        sql: "INSERT INTO test_deadlock_a (id, value) VALUES (1, 'a')",
      });
      await pgEngine.execute({
        sql: "INSERT INTO test_deadlock_b (id, value) VALUES (1, 'b')",
      });
      ...
      try {
        ...
        try {
          await pgEngine.rollback(tx1);
        } catch { /* ignore */ }
        try {
          await pgEngine.rollback(tx2);
        } catch { /* ignore */ }
  ...
  await t.step('error handling and recovery scenarios', async (u) => {
    await u.step('should handle SQL syntax errors gracefully', async () => {
      ...
      const recovery = await pgEngine.execute({
    ...
    await u.step('should handle constraint violations', async () => {
      ...
      await pgEngine.execute({
        sql:
          'CREATE TABLE test_constraints (id INTEGER PRIMARY KEY, name TEXT UNIQUE)',
      });

      // Insert initial data
      await pgEngine.execute({
        sql:
          "INSERT INTO test_constraints (id, name) VALUES (1, 'unique_name')",
      });
      ...
      try {
        await pgEngine.execute({
          sql:
            "INSERT INTO test_constraints (id, name) VALUES (2, 'unique_name')",
        });
  ...
  await t.step('misconfiguration and edge cases', async (u) => {
    ...
    await u.step('should handle very long queries', async () => {
      ...
      const longQuery = 'SELECT ' +
        Array(1000).fill(0).map((_, i) => `1 as col${i}`).join(', ');
    ...
    await u.step('should handle rapid connect/disconnect cycles', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        asserts.assert(
          cycleEngine.status === 'IDLE' || cycleEngine.status === 'RUNNING',
        );
