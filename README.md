...
| [cacher](packages/cacher/README.md)       |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | Memory engine works everywhere. **Workers:** Redis and Memcached now connect over `compat/net`'s real `cloudflare:sockets` backend (compat 2.6.0). **Browser:** Memory only — Redis and Memcached need real TCP, which a browser does not have.                                                                                                                                                                                                                                                                                                                                                               |
| [compat](packages/compat/README.md)       |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | **Workers (since 2.6.0):** outbound TCP/TLS (`net.connect`/`upgradeTls`, via `cloudflare:sockets`), file I/O under `/tmp` (`makeTempFile`/`makeTempDir` need `{ allowEphemeral: true }`), and `WebSocketServer.handleUpgrade()` all work. Still unsupported: `net.listen()`, `./udp`, `./watch`, `WebServer.start()`/`.listen()`, `./cli`'s subprocess spawning, and `file`'s directory/copy/move/rename/`realPath`/`openFile` ops. **Browser:** all of the above throw, plus `./websocket` entirely; `./runtime`, `./fetch`, `./http`, `./path`, `./common` and `./permissions` work.                        |
| [cronus](packages/cronus/README.md)       |  ✅  | ✅  |  ✅  |   ✅    |   ✅    | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [crypt](packages/crypt/README.md)         |  ✅  | ✅  |  ✅  |   ✅    |   ✅    | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [doctor](packages/doctor/README.md)       |  ✅  | ✅  |  ✅  |   ✅    |   ✅    | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [drivers](packages/drivers/README.md)     |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | Import engines by subpath. **Workers:** all four TCP engines now work — `./postgres`, `./redis` and `./memcached` over `compat/net`'s real `cloudflare:sockets` backend (compat 2.6.0), and `./maria` via the third-party `mariadb` driver, which bypasses `compat` entirely and always worked there. The fetch-based `./d1`, `./neon` and `./turso` also work. Unavailable: `./sqlite` (native binding) and `./mongo` (`npm:mongodb`, unverified on Workers). **Browser:** only `./d1`, `./neon` and `./turso` — the rest need real TCP or Node globals a browser doesn't have.                              |
...
| [slogger](packages/slogger/README.md)     |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | Console, Memory and HTTP handlers work everywhere. **Workers:** `FileHandler` can write under `/tmp` now (compat 2.6.0), but a write does not survive past the request that made it — there is no isolate-lifetime `/tmp` to speak of, only a per-request one. TCP and Syslog handlers still need a host runtime. **Browser:** File, TCP and Syslog handlers cannot run at all — no filesystem or raw sockets there.                                                                                                                                                                                          |
...
Workers, confirmed two independent ways: `compat/net` has its own
`cloudflare:sockets` backend (since 2.6.0) that `postgres`/`redis`/`memcached`
connect through, and a driver that manages its own sockets — `drivers/maria`'s
third-party `mariadb` client — reaches the same result via Cloudflare's
