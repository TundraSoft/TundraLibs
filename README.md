...
| [slogger](packages/slogger/README.md)     |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | Console, Memory and HTTP handlers work everywhere. **Workers:** TCPHandler and SyslogHandler's TCP transport connect via `cloudflare:sockets` (needs `nodejs_compat`); a `/tmp` FileHandler path writes to an ephemeral in-memory filesystem instead — records are lost when the isolate recycles, and the handler warns; SyslogHandler's UDP and UNIX-socket transports don't work. **Browser:** File, TCP and Syslog all need a host runtime.                                                                                      |
