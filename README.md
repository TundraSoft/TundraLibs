...
| [drivers](packages/drivers/README.md)     |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | Import engines by subpath. **Workers and browser:** only the fetch-based `./d1`, `./neon` and `./turso` work. Postgres, MariaDB, Redis and Memcached all go through `compat/net`'s `connect()`, which throws `UnsupportedRuntimeError` on Workers since compat 2.2.0 (workerd used to be misdetected as Node, letting TCP through by accident — that is now a deliberate block, not a capability). `./sqlite` needs a native binding; `./mongo` needs `npm:mongodb`, which isn't `cloudflare:sockets`-based. |
