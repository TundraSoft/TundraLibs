...
- **[Pact](packages/pact/README.md)** [![JSR](https://jsr.io/badges/@tundralibs/pact)](https://jsr.io/@tundralibs/pact) — Permissions, Authentication, Control & Tokens — a transport-agnostic authentication & authorization toolkit: RBAC BigInt-bitmask permissions, JWT & opaque sessions with refresh-token rotation, five credential schemes (password, JWT, API keys, HMAC, tokens), TOTP MFA, and an OAuth2/OIDC client over flat bring-your-own-storage hooks
...
- **[Rapid](packages/rapid/README.md)** [![JSR](https://jsr.io/badges/@tundralibs/rapid)](https://jsr.io/@tundralibs/rapid) — rAPId — config-driven API framework: Oak-style function assembly (routes, middleware, cron jobs) with native observability (slogger, tracer, ambient correlation). Early development.
