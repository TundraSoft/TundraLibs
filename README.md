...
| [slogger](packages/slogger/README.md)     |  ✅  | ✅  |  ✅  |   ⚠️    |   ⚠️    | Console, Memory and HTTP handlers work everywhere. **Workers:** `FileHandler` now works — reads/writes land in workerd's `/tmp` (via `compat` >= 2.6.0) — but a record is gone by the very next request, not merely when the isolate eventually recycles; the handler detects this at open and warns once per instance. `TCPHandler`/`SyslogHandler` still need a host runtime. **Browser:** no filesystem or raw socket — File, TCP and Syslog all throw.                                                                                                                                                                                                                                      |
