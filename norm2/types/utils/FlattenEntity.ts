...
type a = FlattenEntity<{ id: number[], name: string[], address: { city: string, street: string, zipCode: string, geo: {lat: number, long: number, radius?: number}[] } }>

type NumberOnly<T extends Record<string, unknown> = Record<string, unknown>, Y = FlattenEntity<T>> = {
  [K in keyof Y]: Y[K] extends number ? Y[K] : never
} extends infer O ? { [P in keyof O]: O[P] } : never

type b = NumberOnly<{ id: number[], name: string[], address: { city: string, street: string, zipCode: string, geo: {lat: number, long: number, radius?: number}[] } }>
