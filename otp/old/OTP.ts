...
export class OTP extends Options<TOTPOptions | HOTPOptions> {
   ...
   */
  constructor(options: OptionKeys<OTPOptions>) {
    const defaults: Partial<OTPOptions> = {
      period: 30,
      counter: -30,
      key: OTP.newKey(),
    };
    super(options, defaults);
    this.__mode = this._getOption('type');
  }
...
export class OTP2 extends Options<iOTPOptions> {
  ...
  public async generate(counter?: number): Promise<string> {
    ...
    if (counter === undefined) {
      switch (this._getOption('type')) {
        case 'HOTP':
          if (this._getOption('counter') === undefined) {
            counter = 0;
          } else {
  ...
  public export(): iOTPOptions {
    const config = {
      algo: this._getOption('algo'),
      type: this._getOption('type'),
      length: this._getOption('length'),
      key: this._getOption('key'),
      counter: this._getOption('counter'),
      period: this._getOption('period'),
    };
    if (this._getOption('type') === 'HOTP') {
      delete config.period;
    }
    if (this._getOption('type') === 'TOTP') {
