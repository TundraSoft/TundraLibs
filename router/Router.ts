...
class RouterNode<T extends RouteHandler = RouteHandler> {
  ...
  get fullPath() {
    return ((this._parent) ? `${this._parent.fullPath}${this._path}` : this._path).replace(/\/+/g, '/');
  }
  ...
  add(path: string, method: HTTPMethods = 'GET', handler: T[]) {
    const [cleanPath, _params]: [string, string[]] = this._standardizePath(path);
    ...
    this._children.forEach((child) => {
      ...
      if(commonPath.length > 0 && commonPath.length > (bestMatch?.length || 0)) {
    ...
    if(bestMatch === undefined) {
      ...
      if(bestMatch === bestMatchNode?.path) {
        ...
        delete this._children[this._children.indexOf(bestMatchNode as RouterNode)];
