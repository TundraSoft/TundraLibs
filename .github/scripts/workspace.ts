 ...
 * - `.github/ISSUE_TEMPLATE/ci_health.md` weekly-health failure template
 *
...
/**
 * Codecov config. One `component` per package splits the coverage by path;
 * one `flag` per runtime (deno/bun/node) splits it by source, and the two
 * compose — components report the union across runtimes. Project and patch
 * both target 85% (per-package component statuses stay `auto`, i.e. must not
 * regress, since a package's honest post-merge number may sit under 85%).
 */
const renderCodecov = (pkgs: Pkg[]): string => {
  const lines = [
    '# GENERATED by .github/scripts/workspace.ts — do not edit.',
    '# Components split coverage by package; per-runtime flags split it by',
    '# source (deno/bun/node) and are unioned. Project/patch target 85%.',
    'ignore:',
    "  - '**/*.test.ts'",
    "  - '**/*.bench.ts'",
    "  - '**/tests/**'",
    ...
    '        target: 85%',
    '        threshold: 5%',
    // Per-runtime coverage is uploaded under one flag each (deno/bun/node) and
    // unioned. carryforward keeps a leg's last-known coverage when its upload
    // is absent from a run, so a flaky or skipped leg never drops the union.
    'flag_management:',
    '  default_rules:',
    '    carryforward: true',
    'comment:',
...
/**
 * Classic Markdown template for the weekly-health failure issue. Unlike the
 * bug/feature forms, this one is filed by automation (`.github/workflows/
 * health.yml`) rather than a human — that job reads this file's body verbatim,
 * so it is the single source of truth for the issue's shape. The frontmatter
 * \`title\` must match the title the workflow searches/creates by.
 */
const renderCiHealth = (): string =>
  `<!-- GENERATED by .github/scripts/workspace.ts — do not edit. -->
---
...
const sync = (check: boolean): void => {
  ...
  const outputs: Record<string, string> = {
    ...
    '.github/ISSUE_TEMPLATE/ci_health.md': renderCiHealth(),
