...
Deno.test('guardian.ArrayGuardian', async (t) => {
  ...
  await t.step('element validation with .of()', async (t) => {
    await t.step('should validate string elements', () => {
      const stringArrayGuard = Guardian.array().of(Guardian.string());
    ...
    await t.step('should validate number elements with constraints', () => {
      const positiveNumberArray = Guardian.array().of(
  ...
  await t.step('array validations', async (t) => {
    await t.step('should validate unique elements', () => {
      const uniqueGuard = Guardian.array().of(Guardian.string()).unique();
    ...
    await t.step('should validate element inclusion', () => {
      const includesGuard = Guardian.array().of(Guardian.string()).includes(
        'hello',
      );
    ...
    await t.step('should validate element exclusion', () => {
      const excludesGuard = Guardian.array().of(Guardian.string()).excludes(
        'forbidden',
      );
  ...
  await t.step('transformations', async (t) => {
    await t.step('should map array elements', () => {
      const mappedGuard = Guardian.array()
        .of(Guardian.number())
        .map((x) => x * 2);
    ...
    await t.step('should map with index', () => {
      const mappedWithIndexGuard = Guardian.array()
        .of(Guardian.string())
        .map((x, i) => `${i}: ${x}`);
    ...
    await t.step('should filter array elements', () => {
      const filteredGuard = Guardian.array()
        .of(Guardian.number())
        .filter((x) => x > 2);
    ...
    await t.step('should sort array elements', () => {
      const sortedGuard = Guardian.array()
        .of(Guardian.number())
        .sort();
    ...
    await t.step('should reverse array elements', () => {
      const reversedGuard = Guardian.array()
        .of(Guardian.string())
        .reverse();
  ...
  await t.step('chained validations', async (t) => {
    await t.step('should chain multiple array validations', () => {
      const complexGuard = Guardian.array()
        .of(Guardian.string().minLength(2))
        .minLength(2)
        .maxLength(5)
        .unique();
    ...
    await t.step('should maintain type safety through chaining', () => {
      const typedGuard = Guardian.array()
        .of(Guardian.string())
        .map((s) => s.length)
        .filter((n) => n > 3);
  ...
  await t.step('safe parsing', async (t) => {
    await t.step('should return success result for valid input', () => {
      const arrayGuard = Guardian.array().of(Guardian.string());
  ...
  await t.step('error handling', async (t) => {
    await t.step('should provide detailed error messages', () => {
      const stringArrayGuard = Guardian.array().of(
    ...
    await t.step('should preserve error context', () => {
      const arrayGuard = Guardian.array().of(Guardian.number().min(10));
      try {
  ...
  await t.step('complex scenarios', async (t) => {
    await t.step('should handle nested arrays', () => {
      const nestedArrayGuard = Guardian.array().of(
        Guardian.array().of(Guardian.number()),
      );
    ...
    await t.step('should validate array of emails', () => {
      const emailArrayGuard = Guardian.array()
        .of(Guardian.string().regex(/^[^\s@]+@[^\s@]+\.[^\s@]+$/))
        .minLength(1)
        .unique();
    ...
    await t.step('should handle complex transformation pipeline', () => {
      const pipelineGuard = Guardian.array()
        .of(Guardian.string().minLength(1))
        .nonEmpty()
  ...
  await t.step('metadata handling', async (t) => {
    await t.step('should store and retrieve metadata', () => {
      ...
      const arrayGuard = Guardian.array(metaData);
  ...
  await t.step('real world usage', async (t) => {
    await t.step('should validate shopping cart items', () => {
      ...
      const cartGuard = Guardian.array()
        .of(Guardian.number().positive().integer())
        .minLength(1)
        .maxLength(50)
        .unique();
    ...
    await t.step('should validate tag system', () => {
      const tagGuard = Guardian.array()
        .of(Guardian.string().minLength(1).maxLength(20))
        .minLength(1)
        .maxLength(10)
        .unique()
