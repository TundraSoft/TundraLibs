...
> extends BaseGuardian<FunctionType<string, P>> {
   ...
   *
   * @returns {GuardianProxy<this>} A GuardianProxy instance that applies the transformation.
   */
  ...
  camelCase(): GuardianProxy<this> {
    return this.transform((str: string) => {
      ...
      return words.shift()?.toLowerCase() +
  ...
  /**
   * Replace a string with the specified string (Only first occurance)
   *
   * @param {string} search to search for
   * @param {string} replace to replace with
   * @returns {GuardianProxy<this>}
   */
  replace(
    ...
    // If it is regexp, and it is all, ensure global flag is set
   ...
   * @param {string} format The format of the date string. Defaults to 'YYYY-MM-dd'.
   * @returns {GuardianProxy<this>} A GuardianProxy instance with a DateGuardian as its type parameter.
   */
  ...
  ipv6(message?: string): GuardianProxy<this> {
    return this.pattern(
      ...
      message || `Expect string to be a valid IPv6 address`,
    );
  }
  ...
  url(message?: string): GuardianProxy<this> {
    ...
    return this.pattern(
      /^(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})|(https?:\/\/localhost(\:[0-9]{2,5})?)$/i,
