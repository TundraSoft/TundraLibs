...
import { assertEquals, describe, it, assertThrows } from '../../dev.dependencies.ts';
...
describe('Syslog', () => {
  ...
  assertEquals(syslog.dateTime.getTime(), new Date('2022-01-01T00:00:00Z').getTime());
  ...
  assertThrows(() => { (syslog as Record<string, unknown>).version = '2'; }, Error, 'Invalid version');
  assertThrows(() => { syslog.pri = 192; }, Error, 'Invalid pri');
  assertThrows(() => { (syslog as Record<string, unknown>).dateTime = 'invalid'; }, Error, 'Invalid dateTime provided');
  assertThrows(() => { (syslog as Record<string, unknown>).severity = 8; }, Error, 'Invalid severity');
  assertThrows(() => { (syslog as Record<string, unknown>).facility = 24; }, Error, 'Invalid facility');
  assertThrows(() => { syslog.structuredData = { 'invalid': { key: 'value' } }; }, Error, 'Invalid structuredData id invalid');
  assertThrows(() => { syslog.structuredData = { 'example@123': { 'invalid key': 'value' } }; }, Error, 'Invalid structuredData key invalid key');
  assertThrows(() => { syslog.structuredData = { 'example@123': { key: ' invalid value ' } }; }, Error, 'Invalid structuredData value');
  });
});
