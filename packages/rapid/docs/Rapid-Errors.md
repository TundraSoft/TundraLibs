...
- **`app.onError` runs in the request's ambient scope** and may return a
  ...
  default envelope is used. It receives the FULL `RapidError` (`debug`
  included) and its return is sent as-is — build the body from
  `error.payload(app.mode)` unless you mean to disclose more.
...
| `RAPID_CONFIG`                  | 500    | Internal server error                                    | A bad option at boot or at a factory (`cors({ maxAge: -1 })`, `timeout(0)`, an unknown `authorize()` module). Usually at `initialize()`/`start()` or when a factory is called; a few misuses surface per request instead: a signing feature (`session()`, `csrf()`, `{ signed: true }`) with no app `secret`, a second `setAuth()`, `fetch()` on an app with socket commands. |
...
| `RAPID_NOT_FOUND`               | 404    | Not found                                                | No route matched — including a `uiOnly` or UI runtime route requested on the api surface.                                                                                                                                                                                                                                                                                     |
...
`details` is a plain JSON-serialisable object (a BigInt fails serialisation
and turns the response into a 500; functions and `undefined` are silently
dropped).
...
(a `{ status: 422, content: {...} }` returned from the handler) gets the
registry code for that exact status (`409` → `RAPID_CONFLICT`, `404` →
`RAPID_NOT_FOUND`); a status with no registry code falls back by class —
`RAPID_VALIDATION_FAILED` for 4xx, `RAPID_UNHANDLED` for 5xx — and keeps its
content as `data`, so a socket
