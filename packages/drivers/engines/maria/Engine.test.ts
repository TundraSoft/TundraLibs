...
const TEST_CONFIG = {
  ...
  // parallel against the same container (Option A); local keeps MARIA_DB.
...
async function isMariaAvailable(): Promise<boolean> {
  const probe = new MariaEngine('maria-probe', TEST_CONFIG);
  try {
    await probe.connect();
...
const tableName = (label: string): string =>
  `tundra_maria_${label.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}_${++_t}`;

// ---------------------------------------------------------------------------
// Engine-specific behaviour.
// ---------------------------------------------------------------------------
describe('drivers.MariaEngine', () => {
  describe('configuration', () => {
    it('should expose Engine and Capabilities', () => {
      const engine = new MariaEngine('cfg-1', TEST_CONFIG);
      ...
      asserts.assertThrows(
        () =>
          new MariaEngine(
            'cfg-db',
            // deno-lint-ignore no-explicit-any
            { host: TEST_CONFIG.host, username: 'u' } as any,
          ),
        EngineError,
        'database',
      );
      asserts.assertThrows(
        () =>
          new MariaEngine(
            'cfg-user',
            // deno-lint-ignore no-explicit-any
            { host: TEST_CONFIG.host, database: 'd' } as any,
          ),
        EngineError,
        'username',
      );
...
// (`{ code, errno, message }`) through the protected `_wrapDriverError` and
// asserts the resulting `EngineError.code`, covering the `_mariaErrorToCode`
// branches the live suite rarely reaches (DEADLOCK, LOCK_TIMEOUT,
// QUERY_TIMEOUT, CONNECTION_LOST, PERMISSION_DENIED, INVALID_AUTH-by-code, …).
// ---------------------------------------------------------------------------
class ProbeMariaEngine extends MariaEngine {
  public wrapError(error: unknown, query: EngineQuery): EngineError {
    return this._wrapDriverError(error, query);
  }
}

describe('drivers.MariaEngine.error-mapping (no DB)', () => {
  const engine = new ProbeMariaEngine('maria-errmap', TEST_CONFIG);
  const q: EngineQuery = { sql: 'SELECT 1' };
  const codeFor = (driverCode: string): string =>
    engine.wrapError(
      { code: driverCode, message: `synthetic ${driverCode}` },
      q,
    ).code;

  it('maps driver error codes to EngineError codes', () => {
    const cases: Array<[string, string]> = [
      ['ER_ACCESS_DENIED_ERROR', 'INVALID_AUTH'],
      ['ER_PASSWORD_NOT_ALLOWED', 'INVALID_AUTH'],
      ...
      ['ER_STATEMENT_TIMEOUT', 'QUERY_TIMEOUT'],
    ...
    for (const [driverCode, expected] of cases) {
      asserts.assertEquals(
        codeFor(driverCode),
        expected,
        `${driverCode} should map to ${expected}`,
      );
    }
  });

  it('maps an unknown driver code to QUERY_EXECUTION_FAILED', () => {
    asserts.assertEquals(codeFor('ER_SOMETHING_NEW'), 'QUERY_EXECUTION_FAILED');
  });

  it('carries driver metadata onto the wrapped error', () => {
    ...
    asserts.assertEquals(err.code, 'DEADLOCK');
    asserts.assertEquals(
      (err.context as Record<string, unknown>).driverErrno,
      1213,
    );
  });

  it('returns an EngineError argument unchanged', () => {
    const original = new EngineError('LOCK_TIMEOUT', {
      instanceId: 'MARIA::maria-errmap',
    });
...
describe({
  name: 'drivers.MariaEngine.engine-specific',
  ignore: !mariaAvailable,
  fn: () => {
    describe('authentication', () => {
      it('should reject bad password', async () => {
        const engine = new MariaEngine('auth-bad', {
          ...TEST_CONFIG,
    ...
    describe('MariaDB-specific behaviour', () => {
      it('returns BIGINT counts as bigint with supportBigInt: true', async () => {
        const engine = new MariaEngine('big-1', TEST_CONFIG);
        const r = await engine.execute<{ n: bigint }>({
          sql: 'SELECT COUNT(*) AS n FROM information_schema.TABLES',
        });
        asserts.assertEquals(typeof r.data[0]?.n, 'bigint');
        await engine.disconnect();
      });
    });
  },
...
// ---------------------------------------------------------------------------
describe({
  name: 'drivers.MariaEngine.sql',
  ignore: !mariaAvailable,
  fn: () => {
    describe('lifecycle', () => {
      it('should connect, ping, and disconnect', async () => {
        const engine = new MariaEngine('maria-life-1', TEST_CONFIG);
        await engine.connect();
      ...
      it('should be idempotent on repeated connect/disconnect', async () => {
        const engine = new MariaEngine('maria-life-2', TEST_CONFIG);
        await engine.connect();
        await engine.connect();
        await engine.disconnect();
        await engine.disconnect();
      ...
      it('should auto-connect on first execute', async () => {
        const engine = new MariaEngine('maria-life-3', TEST_CONFIG);
        asserts.assertEquals(engine.status, 'CLOSED');
    ...
    describe('DDL', () => {
      it('should CREATE and DROP a table', async () => {
        const engine = new MariaEngine('maria-ddl-1', TEST_CONFIG);
        await engine.connect();
      ...
      it('DROP TABLE IF EXISTS should not error on missing table', async () => {
        const engine = new MariaEngine('maria-ddl-2', TEST_CONFIG);
        await engine.connect();
        await engine.execute({
    ...
    describe('CRUD on users table', () => {
      const seed = async (engine: MariaEngine, t: string): Promise<void> => {
        await engine.execute({
          sql: `CREATE TABLE ${t} (
            id INT PRIMARY KEY,
      ...
      it('INSERT should report affected row count', async () => {
        const engine = new MariaEngine('maria-crud-insert', TEST_CONFIG);
        await engine.connect();
      ...
      it('SELECT * should return all rows', async () => {
        const engine = new MariaEngine('maria-crud-select-all', TEST_CONFIG);
        await engine.connect();
      ...
      it('SELECT with WHERE filters by named param', async () => {
        const engine = new MariaEngine('maria-crud-where', TEST_CONFIG);
        await engine.connect();
      ...
      it('SELECT with ORDER BY DESC', async () => {
        const engine = new MariaEngine('maria-crud-orderby', TEST_CONFIG);
        await engine.connect();
      ...
      it('SELECT with LIMIT', async () => {
        const engine = new MariaEngine('maria-crud-limit', TEST_CONFIG);
        await engine.connect();
      ...
      it('UPDATE should report rows affected', async () => {
        const engine = new MariaEngine('maria-crud-update', TEST_CONFIG);
        await engine.connect();
      ...
      it('DELETE should remove matching rows', async () => {
        const engine = new MariaEngine('maria-crud-delete', TEST_CONFIG);
        await engine.connect();
      ...
      it('TRUNCATE should empty the table', async () => {
        const engine = new MariaEngine('maria-crud-truncate', TEST_CONFIG);
        await engine.connect();
    ...
    describe('type round-trips', () => {
      it('string / integer / null', async () => {
        const engine = new MariaEngine('maria-types-1', TEST_CONFIG);
        await engine.connect();
        ...
        try {
          ...
          // Maria with supportBigInt:true may surface aggregates as bigint;
          // for INT columns it returns plain number — fall back to Number()
          // for safety.
          asserts.assertEquals(Number(row.num), 42);
      ...
      it('utf-8 / multi-byte values round-trip', async () => {
        const engine = new MariaEngine('maria-types-utf8', TEST_CONFIG);
        await engine.connect();
    ...
    describe('error mapping', () => {
      it('TABLE_NOT_FOUND on undefined table', async () => {
        const engine = new MariaEngine('maria-err-table', TEST_CONFIG);
        await engine.connect();
        try {
          await engine.execute({
      ...
      it('COLUMN_NOT_FOUND on undefined column', async () => {
        const engine = new MariaEngine('maria-err-column', TEST_CONFIG);
        await engine.connect();
      ...
      it('SYNTAX_ERROR on bad SQL', async () => {
        const engine = new MariaEngine('maria-err-syntax', TEST_CONFIG);
        await engine.connect();
        try {
      ...
      it('DUPLICATE_KEY on PK violation', async () => {
        const engine = new MariaEngine('maria-err-dup', TEST_CONFIG);
        await engine.connect();
      ...
      it('NOT_NULL_VIOLATION on missing required column', async () => {
        const engine = new MariaEngine('maria-err-nn', TEST_CONFIG);
        await engine.connect();
      ...
      it('MISSING_PARAMETERS when a placeholder lacks a param', async () => {
        const engine = new MariaEngine('maria-err-params', TEST_CONFIG);
        await engine.connect();
        try {
    ...
    describe('transactions', () => {
      it('commit persists changes', async () => {
        const engine = new MariaEngine('maria-tx-commit', TEST_CONFIG);
        await engine.connect();
      ...
      it('rollback discards changes', async () => {
        const engine = new MariaEngine('maria-tx-rollback', TEST_CONFIG);
        await engine.connect();
      ...
      it('auto-rollback on query failure inside transaction', async () => {
        const engine = new MariaEngine('maria-tx-auto', TEST_CONFIG);
        await engine.connect();
      ...
      it('idempotent commit/rollback after end', async () => {
        const engine = new MariaEngine('maria-tx-idem', TEST_CONFIG);
        await engine.connect();
        const tx = await engine.transaction();
        await tx.commit();
        await tx.commit();
    ...
    describe('transaction API forms', () => {
      it('beginTransaction + execute(transactionId) + commitTransaction', async () => {
        const engine = new MariaEngine('maria-txform-1', TEST_CONFIG);
        await engine.connect();
      ...
      it('beginTransaction + execute(transactionId) + rollbackTransaction', async () => {
        const engine = new MariaEngine('maria-txform-2', TEST_CONFIG);
        await engine.connect();
      ...
      it('named transactions: beginTransaction({name}) reuses the supplied id', async () => {
        const engine = new MariaEngine('maria-txform-3', TEST_CONFIG);
        await engine.connect();
        try {
      ...
      it('TRANSACTION_NOT_FOUND when execute references an unknown transactionId', async () => {
        const engine = new MariaEngine('maria-txform-4', TEST_CONFIG);
        await engine.connect();
        try {
          await engine.execute({
            sql: 'SELECT 1',
      ...
      it('autoRollbackOnFailure rolls back the txn when a query fails inside it', async () => {
        const engine = new MariaEngine('maria-txform-5', TEST_CONFIG);
        await engine.connect();
      ...
      it('rollbackAllTransactions clears every active transaction', async () => {
        const engine = new MariaEngine('maria-txform-6', {
          ...TEST_CONFIG,
          pool: { min: 2, max: 4 },
        });
        await engine.connect();
      ...
      it('transactionTimeout auto-rolls back a stale transaction', async () => {
        const engine = new MariaEngine('maria-txform-7', TEST_CONFIG);
        await engine.connect();
    ...
    describe('stats and events', () => {
      it('query stats accumulate', async () => {
        const engine = new MariaEngine('maria-stat-1', TEST_CONFIG);
      ...
      it('failed queries are counted', async () => {
        const engine = new MariaEngine('maria-stat-2', TEST_CONFIG);
        try {
          await engine.execute({ sql: 'SELEKT 1' });
        } catch {
          /* expected */
      ...
      it('query event fires for every execute', async () => {
        const events: number[] = [];
        const engine = new MariaEngine('maria-evt-1', TEST_CONFIG);
        // deno-lint-ignore no-explicit-any
