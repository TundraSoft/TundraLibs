...
describe(
  ...
  () => {
    it('should create a CacherError instance', () => {
    ...
    it('should handle message with template variables', () => {
      const meta = {
        name: 'cache1',
        engine: 'memory',
        operation: 'get',
    ...
    it('should handle nested CacherError causes', () => {
    ...
    it('should serialize with cause to JSON correctly', () => {
      const meta = { name: 'cache1', engine: 'memory' };
    ...
    it('should serialize with nested CacherError cause', () => {
      const meta1 = { name: 'cache1', engine: 'memory' };
      const meta2 = { name: 'cache2', engine: 'redis' };
    ...
    it('should handle minimum required metadata', () => {
    ...
    it('should inherit from BaseError correctly', () => {
      const meta = { name: 'cache1', engine: 'memory' };
    ...
    it('should handle circular references in metadata safely', () => {
      // CacherError stores the metadata by reference and does not
      // traverse it at construction time. A circular reference is
      // therefore retained intact: construction must not throw, the
      // error must remain usable, and accessors that touch only the
      // top-level fields must return the expected values.
      //
      // The caller is on the hook for full serialization (e.g.
      // JSON.stringify(error.toJSON())) — that throws on circulars
      // per the JSON spec, which is acceptable.
      ...
      const error = new CacherError('Circular ref test', meta);
      asserts.assertInstanceOf(error, CacherError);
      asserts.assertEquals(error.message, 'Circular ref test');
      asserts.assertEquals(error.getContextValue('name'), 'cache1');
      asserts.assertEquals(error.getContextValue('engine'), 'memory');
      asserts.assertStrictEquals(error.getContextValue('self'), meta);
    });

    it('should preserve stack trace correctly', () => {
