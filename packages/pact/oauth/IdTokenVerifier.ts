 ...
 * discovery document omits `userinfo_endpoint`) hand PACT the user's identity
 * inside the `id_token` returned by the code→token exchange. That exchange is
 * a direct server-to-server TLS POST, so OIDC Core §3.1.3.7 permits skipping
 * signature validation — which is what this client used to do. Checking the
 * signature against the provider's published key set is defense-in-depth: it
 * removes the token endpoint's *response body* from the trusted computing base
 * (a misbehaving proxy, a mis-issued certificate, a redirected egress path).
 *
 * ## Division of labour with `@tundralibs/crypt`
 *
 * Everything cryptographic belongs to crypt. `verifyJWT` accepts a JWKS entry
 * as a JWK directly, validates that key *against the operation* (family, curve,
 * hash, public-vs-private, `use`/`key_ops`/`alg`), binds the verification
 * primitive to the key's shape rather than the token's header, checks the
 * signature, and validates `iss`/`aud`/`exp`/`nbf`.
 *
 * This module owns only what crypt has no view of: fetching and caching the key
 * set, selecting by `kid`, refreshing on rotation, the OIDC-specific claims RFC
 * 7519 does not define (`azp`, `nonce`) plus the requirement that `exp` be
 * *present*, and the availability policy below.
 *
 * This module used to verify on Web Crypto itself, because crypt's `verifyJWT`
 * was PEM-key-only and had no `ES*`. Both gaps are closed, so the duplicate
 * implementation is gone.
 *
 * ## Algorithm confusion
 *
 * The verification algorithm is taken from the **JWKS entry**, never from the
 * token header. When the JWK declares `alg` that single value is pinned, so a
 * header claiming anything else is refused; when it does not, the pin is the
 * asymmetric allow-list and crypt's key-shape binding rejects any algorithm the
 * key cannot carry. Symmetric algorithms (`HS*`) and `none` are never in the
 * pin — a JWKS key is public, so an `HS256` token "signed" with the public key
 * bytes is a forgery attempt by construction.
 *
 * @module
 */
...
/**
 * Floor between forced (unknown-`kid`) JWKS refreshes. Without a cooldown, a
 * caller replaying tokens carrying random `kid`s would turn every login into
 * an outbound fetch and make PACT an amplifier against the provider's key
 * endpoint.
 */
 ...
 * never inspected here: it is handed to crypt, which validates it against the
 * operation and reports anything unusable as `INVALID_SECRET`.
 */
type RemoteJWK = {
  kid?: unknown;
  alg?: unknown;
...
export class IdTokenVerifier {
   ...
   * rotation is always picked up immediately while a flood of bogus `kid`s
   * still cannot drive more than one refetch per window.
   */
  ...
  /**
   * Decode an `id_token`, verify its signature against the provider's JWKS,
   * validate the standard claims, and return the payload.
   *
   * Claim validation runs on **both** paths — a token that skipped signature
   * verification because the key set was unreachable is still checked for
   * `iss`/`aud`/`exp`/`nbf`/`nonce`, so the degraded path is strictly
   * stronger than plain decoding. That is why {@link __validateClaims} stays
   * here rather than being left entirely to crypt, which only sees the tokens
   * that reached a key.
   *
   ...
   * @throws {@link PactOAuthError} `OAUTH_PROFILE_FAILED` when the provider
   *   returned no `id_token` or it could not be decoded;
   *   `OAUTH_IDTOKEN_INVALID` on a bad signature, an unusable key, a
   *   disallowed/mismatched `alg`, or a failed claim — always fatal, under
   *   either policy; `OAUTH_JWKS_UNAVAILABLE` when the key set could not be
   *   obtained and the policy is `'required'`.
   */
  async verify(
  ...
  /**
   * Verify the token's signature against the resolved JWKS key, delegating the
   * cryptography to crypt's `verifyJWT`. Resolves quietly (after notifying)
   * when the key set is unavailable and the policy is `'preferred'`; throws on
   * every failure crypt reports, regardless of policy.
   *
   * @throws {@link PactOAuthError} `OAUTH_IDTOKEN_INVALID` for anything crypt
   *   refuses; `OAUTH_JWKS_UNAVAILABLE` under the `'required'` policy when the
   *   key set could not be obtained.
   */
  private async __verifySignature(
    ...
    if (jwk === undefined) {
      // Indistinguishable from a mid-rotation token, so it is classified as
      // unavailability rather than forgery — an attacker gains nothing,
      // because a made-up `kid` still cannot produce a valid signature under
      // any key we hold.
      return this.__degrade(
        kid === undefined
    ...
    // SECURITY: the algorithm comes from the key, not the token. A JWKS entry
    // that names an algorithm outside the asymmetric allow-list is refused
    // here rather than pinned, so a key set carrying (say) an `oct`/`HS256`
    // entry can never select a symmetric primitive against published material.
    ...
    try {
      await verifyJWT(idToken, jwk as unknown as JsonWebKey, {
        ...
        // disagrees is then rejected outright. Otherwise pin the allow-list,
        // and let crypt's key-shape and curve binding refuse any algorithm
        // this key cannot carry. Pinning `ES256` also pins P-256.
      ...
      // policies. The key set was obtained, so nothing past this point is an
      // availability problem: `INVALID_SIGNATURE` is a forgery and
      // `INVALID_SECRET` is a key the provider published but crypt refuses to
      // use — degrading either one would convert an attack, or a broken trust
      // anchor, into a silent pass. Degradation is decided *above*, before any
      // key is resolved.
      throw this.__reject(
  ...
  /**
   * Apply the availability policy: hard-fail under `'required'`, notify and
   * continue (claims still checked) under `'preferred'`.
   */
  ...
  /** Fetch + cache the JWKS, coalescing concurrent callers onto one request. */
  private __fetchKeys(uri: string): Promise<RemoteJWK[]> {
    ...
    // Clear the slot however it settles; a rejection must not be cached as a
    // permanently-failing promise.
    return request.finally(() => {
      this.__inflight = undefined;
    });
  }

  /** Build an `OAUTH_IDTOKEN_INVALID` error — always fatal, never degraded. */
  private __reject(
    detail: string,
  ...
  /**
   * Validate the standard OIDC claims. Runs whether or not the signature was
   * checked — these are offline checks and cost nothing.
   *
   * crypt re-checks `iss`/`aud`/`exp`/`nbf` inside `verifyJWT`, but only for
   * tokens that reached a key, and RFC 7519 leaves it free to ignore a
   * *missing* `exp` entirely. So this stays the authority: it is what the
   * degraded path relies on, and it covers the three things crypt does not —
   * a mandatory `exp`, OIDC's `azp`, and the `nonce` replay guard.
   *
   * @throws {@link PactOAuthError} (`OAUTH_IDTOKEN_INVALID`) on any mismatch.
   */
