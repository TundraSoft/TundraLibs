...
describe('crypt.generators.key', () => {
  it('generateRSAKeyPair - RSA-OAEP encryption keys', async () => {
  ...
  it('generateRSAKeyPair - RSA-PSS signing keys', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-PSS',
      keySize: 2048,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateRSAKeyPair - different key sizes', async () => {
    // Test 3072-bit key
  ...
  it('generateRSAKeyPair - with PEM export format', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 2048,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateECKeyPair - ECDSA keys', async () => {
  ...
  it('generateECKeyPair - ECDH keys', async () => {
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDH',
      curve: 'P-384',
    });
  ...
  it('generateECKeyPair - different curves', async () => {
    // Test P-521 curve
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-521',
  ...
  it('generateECKeyPair - with JWK export format', async () => {
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-256',
      format: 'JWK',
  ...
  it('generateKeyPair - convenience function RSA', async () => {
  ...
  it('generateKeyPair - convenience function ECDSA', async () => {
  ...
  it('generateRSAEncryptionKeys - convenience function', async () => {
  ...
  it('generateRSASigningKeys - convenience function', async () => {
  ...
  it('generateECDSAKeys - convenience function', async () => {
  ...
  it('generateECDHKeys - convenience function', async () => {
  ...
  it('Error handling - invalid parameters', async () => {
  ...
  it('Performance test - key generation speed', async () => {
  ...
  it('generateRSAKeyPair - SHA-384 hash', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 2048,
    ...
    assertEquals(keyPair.publicKey.algorithm.name, 'RSA-OAEP');
    assertEquals(
      ((keyPair.publicKey.algorithm as RsaHashedKeyGenParams).hash as {
        name: string;
      }).name,
      'SHA-384',
    );
  ...
  it('generateRSAKeyPair - SHA-512 hash', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-PSS',
      keySize: 2048,
  ...
  it('generateRSAKeyPair - 4096-bit key', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 4096,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateRSAKeyPair - DER export format', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 2048,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateECKeyPair - DER export format', async () => {
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-256',
      format: 'DER',
  ...
  it('generateECKeyPair - PEM export format', async () => {
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-256',
      format: 'PEM',
  ...
  // RAW is public-key-only for EC and must actually return the raw public key.
  // The previous implementation exported the public key and then always threw,
  // so RAW never yielded a value despite being an advertised format.
  it('generateECKeyPair - RAW exports the public key only', async () => {
    const result = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-256',
      format: 'RAW',
    });
    ...
    assert(
      result.publicKeyExported instanceof ArrayBuffer,
      'RAW should export the public key as an ArrayBuffer',
    );
    // Uncompressed P-256 point: 0x04 || X(32) || Y(32) = 65 bytes.
    assertEquals((result.publicKeyExported as ArrayBuffer).byteLength, 65);
    // An EC private key has no raw encoding, so it stays unset (rather than
    // aborting the whole export as the old throw did).
    assertEquals(result.privateKeyExported, undefined);
  });

  // Test unsupported format errors
  it('generateRSAKeyPair - unsupported format error', async () => {
    await assertRejects(
      () =>
        generateRSAKeyPair({
          algorithm: 'RSA-OAEP',
  ...
  it('generateECKeyPair - unsupported format error', async () => {
    await assertRejects(
      () =>
        generateECKeyPair({
          algorithm: 'ECDSA',
  ...
  it('generateRSAKeyPair - non-extractable keys', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 2048,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateECKeyPair - non-extractable keys', async () => {
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-256',
      extractable: false,
  ...
  it('generateRSAKeyPair - format with non-extractable', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 2048,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateECKeyPair - P-384 curve', async () => {
    const keyPair = await generateECKeyPair({
      algorithm: 'ECDSA',
      curve: 'P-384',
    });
  ...
  it('generateKeyPair - RSA-PSS', async () => {
  ...
  it('generateKeyPair - unsupported algorithm', async () => {
    await assertRejects(
      // @ts-expect-error Invalid algorithm
  ...
  it('generateRSAEncryptionKeys - with format', async () => {
  ...
  it('generateRSASigningKeys - with format', async () => {
  ...
  it('generateECDSAKeys - with format', async () => {
  ...
  it('generateECDHKeys - with format', async () => {
  ...
  it('generateRSAKeyPair - JWK export format', async () => {
    const keyPair = await generateRSAKeyPair({
      algorithm: 'RSA-OAEP',
      keySize: 2048,
      hashAlgorithm: 'SHA-256',
  ...
  it('generateRSAEncryptionKeys - default parameters', async () => {
  ...
  it('generateRSASigningKeys - default parameters', async () => {
  ...
  it('generateECDSAKeys - default parameters', async () => {
  ...
  it('generateECDHKeys - default parameters', async () => {
