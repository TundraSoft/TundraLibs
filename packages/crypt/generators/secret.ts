/**
 * @fileoverview Secret and passphrase generation.
 *
 * Generates cryptographically secure secrets, passphrases, and passwords
 * with various encoding options and formats.
 *
 * @module
 *
 * @example
 * ```typescript
 * import { generatePassword, secretGenerator } from '@tundralibs/crypt/generators';
 *
...
import { encodeBase32, encodeBase64, encodeHex } from '@std/encoding';
...
export const secretGenerator = (
  ...
  switch (finalEncoding) {
    ...
    case 'ALPHANUMERIC':
      // Map each output position to an unbiased index in [0, 62) (0-9, A-Z,
      // a-z). Reducing a raw byte with `byte % 62` skews the result: 256 is
      // 62*4 + 8, so residues 0-7 ('0'-'7') come up ~25% more often than the
      // rest. Draw the index with the rejection sampling {@link randomInt}
      // already implements so every character is equiprobable.
      return Array.from(bytes)
        .map(() => {
          const mod = randomInt(0, 61);
...
export function generatePassword(
  ...
  // Helper to get random character from a set. Uses {@link randomInt}'s
  // rejection sampling rather than `byte % charset.length`, which would
  // over-represent the first `256 % charset.length` characters of the set.
  ...
  // Fisher-Yates shuffle for random distribution. `randomInt(0, i)` draws an
  // unbiased index; `byte % (i + 1)` would bias the shuffle (and, for a
  // password longer than 256 chars, a single byte could not even reach every
  // index).
