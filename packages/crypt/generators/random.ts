/**
 * @fileoverview Cryptographically secure random value generation.
 *
 * Provides functions for generating random numbers, strings, and byte arrays
 * using the Web Crypto API for cryptographic security.
 *
 * @module
 *
 * @example
 * ```typescript
 * import { randomFloat, randomInt, randomNumber } from '@tundralibs/crypt/generators';
 *
 * const dice = randomInt(1, 6);
 * const ratio = randomFloat(0, 1);
 * const num = randomNumber({ min: 0, max: 100 });
 * ```
 */
 ...
 *
 * Accumulates the random bytes with BigInt arithmetic. A `<<`-based
 * accumulator would coerce to a 32-bit signed integer and corrupt (or
 * negate) values once the range needs 4 or more bytes, and a plain `* 256`
 * accumulator would lose precision past {@link Number.MAX_SAFE_INTEGER}.
 * BigInt keeps every byte combination exact across the full safe-integer
 * range before the result is mapped back into `[min, max]`.
 */
function randomIntLargeRange(min: number, _max: number, range: number): number {
  ...
  const maxValue = 1n << BigInt(bytesNeeded * 8);
  const maxValid = (maxValue / rangeBig) * rangeBig - 1n;
  ...
  do {
    ...
    // Convert bytes to a BigInt (big-endian) so wide ranges stay exact.
    randomValue = 0n;
    for (let i = 0; i < bytesNeeded; i++) {
      randomValue = (randomValue << 8n) + BigInt(randomBytes[i] ?? 0);
    }
  } while (randomValue > maxValid);

  return min + Number(randomValue % rangeBig);
}
