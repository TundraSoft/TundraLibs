...
describe('id.nanoId', () => {
  ...
  it('Test edge cases for size parameter', () => {
  ...
  it('Test with custom character sets', () => {
  ...
  it('Test distribution of characters', () => {
  ...
  it('Test random bytes regeneration scenario', () => {
  ...
  it('Test edge cases with very large character sets', () => {
  ...
  it('generates large IDs without a QuotaExceededError (F5)', () => {
    // Regression: allocating Uint32Array(step) in one getRandomValues call
    // exceeded Web Crypto's spec-mandated 65536-byte quota for size >= ~6176
    // (default alphabet), throwing a raw QuotaExceededError DOMException. The
    // random source now fills in <=65536-byte chunks.
    const big = nanoID(8192);
    asserts.assertEquals(big.length, 8192);
    asserts.assertMatch(big, /^[a-z0-9_-]+$/i);

    // Well past a single chunk (step ~= 48000 Uint32 -> 3 chunks) and a
    // different alphabet, to exercise the chunking loop.
    const bigger = nanoID(20000, NUMBERS);
    asserts.assertEquals(bigger.length, 20000);
    asserts.assertMatch(bigger, /^[0-9]+$/);
  });

  it('Test consistency across multiple generations', () => {
