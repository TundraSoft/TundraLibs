...
import type { ResponseBody, RESTlerOptions } from '../../../mod.ts';
...
export class JSONPlaceholderAPI extends RESTler {
  ...
  /**
   * Override the request `fetch` seam with a stub for testing. This fixture
   * lives under `tests/fixtures/`, so exposing the seam here is fine.
   */
  ...
  async getPosts(): Promise<Post[]> {
    const response = await this._makeRequest<Post[] & ResponseBody>(
  ...
  async getUsers(): Promise<User[]> {
    const response = await this._makeRequest<User[] & ResponseBody>(
  ...
  async getPostComments(postId: number): Promise<Comment[]> {
    const response = await this._makeRequest<Comment[] & ResponseBody>(
      { path: `/posts/${postId}/comments`, method: 'GET' },
    );
    return response.body || [];
  }
