...
function isDefinition(v: unknown): v is AnyDefinition {
  return typeof v === 'object' && v !== null &&
    ...
    ((v as { type: unknown }).type === 'TABLE' ||
      ...
      (v as { type: unknown }).type === 'AUDIT');
}

/**
 * The audit replicas `Schema()` generates from `M`'s entities, keyed by
 * each one's OWN registry name (`audit.name`) — the runtime counterpart
 * of {@link injectAuditReplicas}. Reuses {@link _UnionToIntersection}
 * (already used to merge multiple schemas) to merge the one-key object
 * each auditing entity contributes; entities with no `audit` contribute
 * nothing (`{}` has no keys, so it drops out silently).
 */
type _AuditEntitiesOf<M> = _Prettify<
  _UnionToIntersection<
    {
      [K in keyof M]: M[K] extends {
        readonly audit: {
          readonly name: infer AN extends string;
          readonly definition: infer D;
        };
      } ? { readonly [P in AN]: D }
...
/**
 * Relocate every TABLE's generated audit replica (`Entity()` already
 * built the full definition — see `buildAudit` in entity.ts) into the
 * registry under its own name, filling in `auditOf` (the source's
 * registry key — unknowable to `Entity()`, which is called before it is
 * ever placed in a `Schema()` object). Mutates `entities` in place.
 *
 * @throws {Error} When an audit replica's name collides with an
 *   existing entity, or with another replica's name.
 */
function injectAuditReplicas(
  scope: string,
  entities: Record<string, AnyDefinition>,
): void {
  const pending: Array<
    { sourceKey: string; name: string; def: AnyDefinition }
  > = [];
  for (const [sourceKey, def] of Object.entries(entities)) {
    const audit = (def as {
      audit?: { name: string; definition: AnyDefinition };
    }).audit;
    if (audit === undefined) continue;
    if (entities[audit.name] !== undefined) {
      throw new Error(
        `${scope}: audit replica '${audit.name}' (from '${sourceKey}') ` +
          `collides with an existing entity of the same name.`,
      );
    }
    if (pending.some((p) => p.name === audit.name)) {
      throw new Error(
        `${scope}: audit replica '${audit.name}' is generated by more ` +
          `than one entity — audit.name must be unique.`,
      );
    }
    pending.push({
      sourceKey,
      name: audit.name,
      def: Object.freeze({ ...audit.definition, auditOf: sourceKey }),
    });
  }
  for (const p of pending) entities[p.name] = p.def;
}
...
>(
  name: N,
  models: M,
): SchemaValue<N, _Prettify<SchemaDefinition<M> & _AuditEntitiesOf<M>>> {
  if (name.trim() === '') {
  ...
  injectAuditReplicas(`Schema('${name}')`, entities);

  validateRegistry(`Schema('${name}')`, entities, true);

  return { name, entities } as SchemaValue<
    N,
    _Prettify<SchemaDefinition<M> & _AuditEntitiesOf<M>>
  >;
}
