...
describe('utils.memoize', () => {
  ...
  it('should memoize the result of an async function', async () => {
    let counter = 0;
  ...
  it('should memoize the result of an async method', async () => {
    class Calculator {
      static counter = 0;
      @Memoize(1)
  ...
  it('async memoize returns a Promise on cache hits (thenable .then/.catch)', async () => {
    // Regression: a cached async call used to return the RAW resolved value
    // instead of a Promise, so `.then()`/`.catch()` on the 2nd call threw
    // `TypeError: ... is not a function`. Every call must be thenable.
    let counter = 0;
    ...
    asserts.assertEquals(await memoizedFn(21), 42);
    // The cached call must still be a Promise.
    const cached = memoizedFn(21);
    asserts.assertInstanceOf(cached, Promise);
    // `.then()` chaining must work on the cached result.
    asserts.assertEquals(await cached.then((v) => v + 1), 43);
    // `.catch()` must be callable on the cached result too.
    asserts.assertEquals(await memoizedFn(21).catch(() => -1), 42);
    // The underlying fn ran exactly once despite the extra cached calls.
    asserts.assertEquals(counter, 1);
  });

  it('@Memoize async method returns a Promise on cache hits', async () => {
    // Same contract for the decorator, which wraps `memoize`.
    class Api {
      runs = 0;
      @Memoize(60)
      async load(id: number): Promise<number> {
        this.runs++;
        await Promise.resolve();
        return id * 10;
      }
    ...
    const api = new Api();
    asserts.assertEquals(await api.load(5), 50);
    const cached = api.load(5);
    asserts.assertInstanceOf(cached, Promise);
    asserts.assertEquals(await cached.then((v) => v + 1), 51);
    asserts.assertEquals(api.runs, 1);
  });

  it('should reset the cache after the timeout', async () => {
    let counter = 0;
    const add = async (a: number, b: number): Promise<number> => {
      // Set a 500ms delay to simulate an async operation
      await new Promise((resolve) => setTimeout(resolve, 250));
  ...
  it('should correctly memoize with complex object arguments', () => {
    let counter = 0;
  ...
  it('should handle cache expiration edge cases', async () => {
    let counter = 0;
    const fn = () => ++counter;

    // Test with small timeout (50ms instead of 1ms for better reliability)
    const memoizedFn = memoize(fn, 0.05); // 50ms timeout

    asserts.assertEquals(memoizedFn(), 1);
    asserts.assertEquals(memoizedFn(), 1); // Should be cached

    // Wait for cache to expire (150ms to ensure timeout passes)
    await new Promise((resolve) => setTimeout(resolve, 150));

    asserts.assertEquals(memoizedFn(), 2); // Should execute again after expiration
  });

  it('should handle cache key creation edge cases', () => {
    let counter = 0;
  ...
  it('should handle synchronous function error cases', () => {
    let counter = 0;
  ...
  it('should handle decorator on various method types', () => {
    class TestClass {
      static counter = 0;
      private value = 0;
  ...
  it('should cache args whose JSON contains the substring "null"', () => {
    // Regression: the previous substring heuristic treated any args whose
    // JSON contained "null" as non-serializable, so the cache key was
    // random every call and nothing was ever cached.
    let counter = 0;
    ...
    asserts.assertEquals(memoized('nullx'), 'NULLX');
    asserts.assertEquals(memoized('nullx'), 'NULLX');
    asserts.assertEquals(counter, 1);
  });

  it('should cache args containing a genuine null value', () => {
    let counter = 0;
    const fn = (o: { a: number | null }): number => {
      counter++;
      return o.a ?? -1;
    };
    const memoized = memoize(fn);
    asserts.assertEquals(memoized({ a: null }), -1);
    asserts.assertEquals(memoized({ a: null }), -1);
    asserts.assertEquals(counter, 1);
  });

  it('@Memoize method `get_foo` and getter `foo` do not share a cache slot', () => {
    // Regression (round-6): the method branch keyed the per-instance cache as
    // `${Class}_${name}` and the getter branch as `${Class}_get_${name}`, so a
    // method literally named `get_foo` and a getter named `foo` both keyed to
    // `X_get_foo` in the SAME `__memoized` map and silently swapped cached
    // results depending on access order.
    class Y {
      @Memoize(1000)
      get foo(): string {
