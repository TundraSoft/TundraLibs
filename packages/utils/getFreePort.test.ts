...
describe('utils.getFreePort', () => {
  it('returns a port within the specified range', async () => {
    const port = await getFreePort({ min: 3000, max: 4000 });
    asserts.assert(port >= 3000 && port <= 4000, 'Port should be within range');
  });

  it('respects excluded ports', async () => {
    const exclude = [3000, 3001, 3002];
    const port = await getFreePort({ min: 3000, max: 3010, exclude });
    asserts.assert(
  ...
  it('works with min equal to max', async () => {
    // We need to pick a port that's likely to be free
    const port = await getFreePort({ min: 9876, max: 9876 });
    asserts.assertEquals(port, 9876, 'Port should equal the specified value');
  });

  it('finds a free port when some ports are busy', async () => {
    ...
    try {
      ...
      for (let p = min; p < max; p++) {
        try {
          const listener = await listen({ port: p });
      ...
      const port = await getFreePort({ min, max });
  ...
  it('uses default range when no options provided', async () => {
  ...
  it('throws when all ports in range are excluded', async () => {
    await asserts.assertRejects(
  ...
  it('ignores excluded ports outside range', async () => {
    const port = await getFreePort({
      min: 4000,
      max: 4000,
  ...
  it('handles empty exclude array', async () => {
    const port = await getFreePort({ min: 8000, max: 8000, exclude: [] });
    asserts.assertEquals(port, 8000, 'Should handle empty exclude array');
  });

  it('handles large exclude list', async () => {
  ...
  it('returns different ports on multiple calls', async () => {
    const ports = new Set();
    for (let i = 0; i < 10; i++) {
      const port = await getFreePort({ min: 10000, max: 20000 });
      ports.add(port);
    }
  ...
  it('throws when no free port found after max attempts', async () => {
    // Testing with a very small range where all ports can be occupied
    const listeners: Listener[] = [];
    try {
  ...
  it('handles boundary port numbers', async () => {
    // Test with port 0 (valid but unusual)
    const port = await getFreePort({ min: 0, max: 0 });
    asserts.assertEquals(port, 0, 'Should handle port 0');

    // Test with max port number
    const highPort = await getFreePort({ min: 65535, max: 65535 });
    asserts.assertEquals(highPort, 65535, 'Should handle port 65535');
  });

  it('handles edge case max port validation', async () => {
    await asserts.assertRejects(
