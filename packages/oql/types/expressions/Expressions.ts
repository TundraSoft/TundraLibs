 ...
 * `CURRENT_TIMESTAMP`, `CURRENT_TIMESTAMPTZ`, `DATE_ADD` — and
 * `UNIX_TIMESTAMP`, which returns a 64-bit INTEGER, not a Date.
 *
...
> = {
  ...
  /**
   * Unix epoch of the current time as a 64-BIT INTEGER — seconds by
   * default, milliseconds with `unit: 'milliseconds'`. The one date
   * expression that returns a number, not a Date: `BIGINT` on Postgres
   * and MariaDB, a 64-bit `INTEGER` on SQLite, a `Long` on MongoDB.
   * Seconds floor the fraction; milliseconds are as exact as the
   * engine's clock (MariaDB reads `NOW(3)` for it).
   *
   * `unit` is a TRANSLATION-TIME switch, not a value — it is never bound
   * as a parameter, so it must be a literal on the node.
   *
   * What JavaScript receives differs by driver, not by this expression:
   * the Postgres driver decodes `BIGINT` as `bigint`, while MariaDB,
   * SQLite and MongoDB hand back a `number`. Coerce at the edge if a
   * multi-dialect app needs one type.
   */
