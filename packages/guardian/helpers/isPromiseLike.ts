 ...
 * - It's an async function or async generator (which returns a Promise when called)
 *
 * This follows the Promises/A+ specification for thenable detection and extends
 * it to include async functions and async generators for practical async/await compatibility.
 *
 * @param value - The value to check if it is promise-like or async function
 * @returns True if the value is promise-like or an async function/generator, false otherwise
 *
 * @example
 * ```ts
 * // Native Promises
...
export function isPromiseLike(value: unknown): value is PromiseLike<unknown> {
  ...
  if (typeof value === 'function') {
    ...
    // This also covers async generators (async*)
    try {
