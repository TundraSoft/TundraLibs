 ...
 * a {@link NumberGuardian}. Importing the sibling class as a VALUE for
 * that makes the guard modules mutually dependent — a legal ESM cycle
 * that native module evaluation handles fine, but that DEADLOCKS once a
 * bundler lowers module initialisers to async functions (which
 * esbuild / Rollup do for the whole graph as soon as anything in it uses
 * top-level await).
 *
 * The fix is to keep every sibling reference type-only — `import type`
 ...
 *
 * This module deliberately imports nothing from its siblings at runtime,
 * so the value-import graph under `guards/` stays acyclic.
 *
 * @module
 */
 ...
 * away (e.g. {@link BooleanGuardian}) never needs to be resolved.
 *
