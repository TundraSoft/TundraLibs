...
-----END PUBLIC KEY-----`;
    ...
    await asserts.assertRejects(
      async () => {
        await verifyRSA("RSA-PSS:2048:SHA-256:EXTRA" as any, publicKeyPEM, testData, signature);
      },
      Error,
      "Invalid RSA mode format",
    );
    ...
    await asserts.assertRejects(
      async () => {
        await verifyRSA("RSA-OAEP:2048:SHA-256" as any, publicKeyPEM, testData, signature);
      },
      Error,
      "Invalid RSA mode format",
    );
    ...
    await asserts.assertRejects(
      async () => {
        await verifyRSA("RSA-PSS::SHA-256" as any, publicKeyPEM, testData, signature);
      },
      Error,
      "Invalid RSA mode format",
    );
    ...
    await asserts.assertRejects(
      async () => {
        await verifyRSA("RSA-PSS:2048:" as any, publicKeyPEM, testData, signature);
      },
      Error,
      "Invalid RSA mode format",
    );
...
-----END PUBLIC KEY-----`;
    ...
    for (const keySize of invalidKeySizes) {
      await asserts.assertRejects(
        async () => {
          await verifyRSA(`RSA-PSS:${keySize}:SHA-256` as any, publicKeyPEM, testData, signature);
        },
        Error,
...
-----END PUBLIC KEY-----`;
    ...
    for (const hash of invalidHashes) {
      await asserts.assertRejects(
        async () => {
          await verifyRSA(`RSA-PSS:2048:${hash}` as any, publicKeyPEM, testData, signature);
        },
        Error,
...
-----END PUBLIC KEY-----`;
    ...
    await asserts.assertRejects(
      async () => {
        await verifyRSA("RSA-PSS:2048:SHA-256", publicKeyPEM, testData, null as any);
      },
      Error,
      "Signature must be a non-empty string",
    );
  ...
  await t.step("verifyRSA - Invalid PEM formats", async () => {
    ...
    await asserts.assertRejects(
      async () => {
        await verifyRSA("RSA-PSS:2048:SHA-256", "not-a-pem-key", testData, signature);
      },
      Error,
      "Invalid PEM public key format",
    );
...
-----END PUBLIC KEY-----`;
        await verifyRSA("RSA-PSS:2048:SHA-256", invalidPEM, testData, signature);
      },
      Error,
      "Invalid PEM public key format",
    );
  ...
  await t.step("verifyRSA - SHA-384 hash algorithm", async () => {
    ...
    const signature = await signRSA("RSA-PSS:2048:SHA-384", privateKeyPEM, testData);
    const isValid = await verifyRSA("RSA-PSS:2048:SHA-384", publicKeyPEM, testData, signature);
  ...
  await t.step("verifyRSA - SHA-512 hash algorithm", async () => {
    ...
    const signature = await signRSA("RSA-PSS:2048:SHA-512", privateKeyPEM, testData);
    const isValid = await verifyRSA("RSA-PSS:2048:SHA-512", publicKeyPEM, testData, signature);
  ...
  await t.step("verify - HMAC Integration", async () => {
    ...
    for (const algo of algorithms) {
      ...
      const isValid = await verify(`HMAC:${algo}` as any, secret, data, signature);
      asserts.assertEquals(isValid, true, `HMAC verification failed for ${algo}`);
    }
  });
});
