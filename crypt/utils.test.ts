...
Deno.test('crypt.utils', async (t) => {
  await t.step('validateKey', async (h) => {
    await h.step('validates key length correctly', () => {
    ...
    await h.step('handles non-ASCII characters correctly', () => {
  ...
  await t.step('deriveKey', async (h) => {
    await h.step('truncates keys that are too long', () => {
    ...
    await h.step('pads keys that are too short', () => {
      const shortKey = 'abcd'; // 4 bytes
      const requiredBytes = 16;
    ...
    await h.step(
      'keeps keys that are exactly the right length unchanged',
      () => {
    ...
    await h.step('handles various byte lengths', () => {
    ...
    await h.step('handles empty input correctly', () => {
      const emptyKey = '';
      const requiredBytes = 16;
