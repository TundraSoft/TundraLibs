...
Deno.test("crypt.generators.bip39", async (t) => {
  ...
  await t.step("generateBIP39Mnemonic - with passphrase", async () => {
    ...
    const sameMnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
    ...
    assert(seedsAreDifferent, "Different passphrases should produce different seeds");
  });
  ...
  await t.step("generateBIP39Mnemonic - custom wordlist", async () => {
    ...
    const customWordlist = Array.from({ length: 2048 }, (_, i) => `word${i.toString().padStart(4, '0')}`);
    ...
    for (const word of result.words) {
      assert(customWordlist.includes(word), `Word ${word} should be from custom wordlist`);
    }
  });

  // Error handling tests
  await t.step("generateBIP39Mnemonic - invalid wordlist length (too short)", async () => {
  ...
  await t.step("generateBIP39Mnemonic - invalid wordlist length (too long)", async () => {
  ...
  await t.step("validateBIP39Mnemonic - invalid word count (too short)", async () => {
  ...
  await t.step("validateBIP39Mnemonic - invalid word count (too long)", async () => {
  ...
  await t.step("validateBIP39Mnemonic - words not in wordlist", async () => {
    const invalidWordMnemonic = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon invalidword";
  ...
  await t.step("validateBIP39Mnemonic - custom wordlist", async () => {
    ...
    assert(!isValidDefault, "Custom wordlist mnemonic should fail with default wordlist");
  });
  ...
  await t.step("mnemonicToSeed - empty passphrase vs no passphrase", async () => {
    ...
    // Should be identical
    for (let i = 0; i < seed1.length; i++) {
      assertEquals(seed1[i], seed2[i], "Empty passphrase should equal no passphrase");
    }
  });
  ...
  await t.step("mnemonicToSeed - unicode passphrase", async () => {
    ...
    for (let i = 0; i < seed.length; i++) {
      assertEquals(seed[i], seed2[i], "Unicode passphrase should be reproducible");
    }
  });
});
