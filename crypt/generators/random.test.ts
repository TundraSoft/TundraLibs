...
Deno.test('randomInt - edge cases and validation', () => {
  // Test edge cases
  ...
  assertThrows(() => randomInt(10, 5), Error, 'Min cannot be greater than max');
  assertThrows(
    () => randomInt(1.5, 5),
    Error,
...
Deno.test('randomInt - statistical properties', () => {
  const min = 1;
  const max = 20;
...
Deno.test('randomFloat - basic functionality', () => {
  const min = 0.0;
  const max = 1.0;
...
Deno.test('randomFloat - validation', () => {
  // Test error conditions
  assertThrows(() => randomFloat(1, 1), Error, 'Min must be less than max');
  assertThrows(() => randomFloat(10, 5), Error, 'Min must be less than max');
});

Deno.test('randomFloat - precision testing', () => {
...
Deno.test('randomNumber - with options', () => {
  // Test integer mode
  ...
  assert(customRange >= 50 && customRange <= 100, 'Custom range out of bounds');
});

Deno.test('randomInt - large range handling', () => {
...
Deno.test('performance - generation speed', () => {
...
Deno.test('security - no predictable patterns', () => {
