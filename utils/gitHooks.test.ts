import { assertSpyCall, assertSpyCalls, spy } from '../dev.dependencies.ts';
import { loadHooks } from './gitHooks.ts';

// Mock Deno APIs
const readDirMock = spy(async function* (path: string | URL) {
...
const chmodMock = spy(async (_path: string | URL, _mode: number) => {});
...
Deno.test('loadHooks processes valid hooks correctly', async () => {
  ...
  for (const hookName of expectedHooks) {
    ...
    assertSpyCalls(readTextFileMock, 2); // Two valid hooks
    assertSpyCall(readTextFileMock, expectedHooks.indexOf(hookName), {
      args: [hookPath],
    });

    // Check remove, writeTextFile, and chmod were called with correct paths and arguments
    assertSpyCall(removeMock, expectedHooks.indexOf(hookName), {
      args: [gitHookPath],
    });
    assertSpyCall(writeFileMock, expectedHooks.indexOf(hookName), {
      args: [gitHookPath, 'mock file content'],
    });
    assertSpyCall(chmodMock, expectedHooks.indexOf(hookName), {
