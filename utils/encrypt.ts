...
export const encrypt = async (
  ...
  const encopt: { name: 'AES-CTR' | 'AES-CBC' | 'AES-GCM', counter?: Uint8Array, length?: number, iv?: Uint8Array } = {
  ...
  const cryptKey = await crypto.subtle.importKey(
    ...
    encrypted = await crypto.subtle.encrypt(encopt, cryptKey, enc.encode(JSON.stringify(message)));
  if(opt?.encoding === 'HEX') {
    return `${hex.encodeHex(encrypted)}:${hex.encodeHex(encopt.iv || encopt.counter || '')}`;
  } else {
    return `${base64.encodeBase64(encrypted)}:${base64.encodeBase64(encopt.iv || encopt.counter || '')}`;
  }
};
...
export const decrypt = async <T extends unknown = unknown>(
  ...
  const spl = message.split(':'),
    iv = encOpt.encoding === 'HEX' ? hex.decodeHex(spl[1]) : base64.decodeBase64(spl[1]), 
    encopt: { name: 'AES-CTR' | 'AES-CBC' | 'AES-GCM', counter?: Uint8Array, length?: number, iv?: Uint8Array } = {
    name: encOpt.algorithm,
  };
  if(encOpt.algorithm === 'AES-CTR') {
    encopt.counter = iv;
  ...
  const cryptKey = await crypto.subtle.importKey(
    ...
    encrypted = encOpt.encoding === 'HEX' ? hex.decodeHex(spl[0]) : base64.decodeBase64(spl[0]),
