...
export class SQLite<T extends SQLiteConfig> extends AbstractClient<T> {
    ...
    protected async _update<T>(
        ...
        await this._client?.queryEntries(qry);
        const result = await this._client?.queryEntries(select.toString());
        return result as T[];
    }
    ...
    protected async _createTable(options: CreateTableOptions): Promise<void> {
        const table = (options.schema !== undefined)
            ...
            columns = Object.keys(options.columns).map((value) => {
                const colName = value;
                // hack for autoincriment
                if (
                    ["SERIAL", "BIGSERIAL", "AUTO_INCRIMENT"].includes(
                        options.columns[value].type,
                    )
                ) {
                    return `${this._quoteColumn(colName)} INTEGER PRIMARY KEY`;
                }
    ...
    public async _getTableDefinition(table: string, schema?: string): Promise<SchemaDefinition> {
        //TODO Implement
        return {
            table: table,
            schema: schema,
