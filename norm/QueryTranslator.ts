...
type QueryType = 'SELECT' | 'INSERT' | 'UPDATE' | 'DELETE' | 'TRUNCATE' | 'CREATE_TABLE' | 'DROP_TABLE' | 'CREATE_VIEW' | 'DROP_VIEW';
...
type SelectQuery<S extends Schema = Record<string, TableDefinition>, K extends keyof S = keyof S> = & QueryDefaults<S, K> & {
...
type InsertQuery<T extends Schema = Record<string, TableDefinition>, K extends keyof T = keyof T> = & QueryDefaults<T, K> & {
  values: {
    [C in keyof T[K]['columns']]?: T[K]['columns'][C] extends { type: unknown } ? ReturnType<typeof DataTypeMap[T[K]['columns'][C]['type']]> : unknown;
  }[];
};

type UpdateQuery<T extends Schema = Record<string, TableDefinition>, K extends keyof T = keyof T>  = & QueryDefaults<T, K> & {
  values: {
    [C in keyof T[K]['columns']]?: T[K]['columns'][C] extends { type: DataType } ? ReturnType<typeof DataTypeMap[T[K]['columns'][C]['type']]> : unknown;
  }
  filter?: Record<string, unknown>;
};

type DeleteQuery<T extends Schema = Record<string, TableDefinition>, K extends keyof T = keyof T>  = & QueryDefaults<T, K> & {
  filter?: Record<string, unknown>;
};
...
export class QueryTranslator {
  ...
  public insert(options: InsertQuery): string {
    ...
    const columnAlias = Object.keys(options.columns), 
      ...
      keys = options.values.map((row) => Object.keys(row)).flat().filter((value, index, self) => self.indexOf(value) === index), 
    ...
    const table = this._fqdn(options.table, options.schema), 
      ...
      returning = Object.entries(options.columns).map(([alias, column]) => `${this._fqdn(column, options.table, options.schema)} AS ${this._escape(alias)}`).join(', '),
      valueRow = `(${options.values.map((row) => keys.map((key) => (row[key] === undefined) ? `NULL` : `'${row[key]}'`).join(', ')).join('), (')})`;
  ...
  public update(options: UpdateQuery): string {
    ...
    const table = this._fqdn(options.table, options.schema), 
      columns = keys.map((column) => this._escape(column)).join(', '),
      returning = Object.entries(options.columns).map(([alias, column]) => `${this._fqdn(column, options.table, options.schema)} AS ${this._escape(alias)}`).join(', '),
      valueRow = Object.entries(options.values).map(([key, value]) => `${this._escape(key)} = ${(value === undefined) ? `NULL` : `'${value}'`}`).join(', '), 
  ...
  protected _fqdn(...value: Array<string | undefined>): string {
    // remove undefined
    return this._escape(value.filter((v) => v !== undefined).reverse().join('.'))
  }
