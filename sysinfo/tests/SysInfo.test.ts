import { assertEquals, assertAlmostEquals, assertNotEquals } from '../../dev.dependencies.ts';
...
Deno.test({
  ...
  assertEquals(memoryInfo.total > 0, true);
  assertEquals(memoryInfo.free > 0, true);
  assertEquals(memoryInfo.available > 0, true);
  assertEquals(memoryInfo.swap.total > 0, true);
  assertEquals(memoryInfo.swap.free > 0, true);
  // Check if the GB conversion is working
  assertAlmostEquals(memGB.total, memoryInfo.total / 1024 / 1024 / 1024);
  assertAlmostEquals(memGB.free, memoryInfo.free / 1024 / 1024 / 1024);
  assertAlmostEquals(memGB.available, memoryInfo.available / 1024 / 1024 / 1024);
  assertAlmostEquals(memGB.swap.total, memoryInfo.swap.total / 1024 / 1024 / 1024);
  assertAlmostEquals(memGB.swap.free, memoryInfo.swap.free / 1024 / 1024 / 1024);
});
...
Deno.test({
  name: `[library='SysInfo' method='getLoadAverage'] Returns empty array if permission as not set`,
  permissions: { sys: false },
}, async () => {
  const loadAverage = await SysInfo.getLoadAverage();
...
Deno.test({
  name: `[library='SysInfo' method='getHostname'] Returns undefined as permission is not set`,
  permissions: { sys: false },
}, async () => {
  const hostname = await SysInfo.getHostname();
