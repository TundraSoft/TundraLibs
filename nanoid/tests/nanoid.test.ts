...
Deno.test({
  name:
    `[library='nanoid' mode='nanoid'] Check for length consistency on sample set of ${sampleSize}}`,
  fn() {
...
Deno.test({
  name:
    `[library='nanoid' mode='nanoid'] Ensure only allowed characters are present on sample set of ${sampleSize}}`,
  fn() {
    for (let i = minLength; i <= maxLength; i++) {
      dictionary.forEach((dict) => {
        for (let j = 0; j < sampleSize; j++) {
...
Deno.test({
  name:
    `[library='nanoid' mode='nanoid'] Check if the collision is < 1% on sample set of  ${sampleSize}}`,
  fn() {
    let id: string;
    for (let i = minLength; i <= maxLength; i++) {
